"""V2 §6 — Final push: process high-yield sources with max_items=100.

The corpus needs ~200 more IOs and ~63 more documents.
Process high-yield RSS sources (euronext, bea, fca, esma, sec, etc.) with
max_items=100 to extract more documents.
"""
...
def final_push(store_root: str = "v3_corpus_store",
                registry_root: str = "source_registry",
                max_workers: int = 6):
    """Process high-yield RSS sources with max_items=100."""
    print(f"\n{'='*70}")
    print(f"V2 §6 — Final Push: High-Yield RSS Sources (max_items=100)")
    print(f"{'='*70}")
    ...
    # Get RSS/ATOM production-ready sources
    rss_pr_sources = [
        r for r in src_registry.all()
        if r.qualification_status == "PRODUCTION_READY"
        and r.acquisition_method in ("RSS", "ATOM")
    ]
    print(f"  RSS/ATOM production-ready sources: {len(rss_pr_sources)}")
    ...
    run_id = f"v3-final-{int(time.time())}"
    t_start = time.perf_counter()

    results = [None] * len(rss_pr_sources)

    def worker(idx, r):
        # Use max_items=100 for high yield
        result = process_new_source(store, institution_registry, r, run_id, max_items=100)
        return idx, result

    future_to_idx = {}
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        for i, r in enumerate(rss_pr_sources):
        ...
        for future in as_completed(future_to_idx):
            ...
            try:
                ...
                print(f"  [{idx+1:2d}/{len(rss_pr_sources)}] {src_id:<30} docs={docs:3d} facts={facts:4d} ios={n_ios:3d}")
...
if __name__ == "__main__":
    final_events, final_docs = final_push()
